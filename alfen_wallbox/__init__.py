...
async def async_migrate_entry(hass: HomeAssistant, config_entry: AlfenConfigEntry) -> bool:
    ...
    if config_entry.version == 2:
        # Add CAT_LOGS and CAT_TRANSACTIONS to saved refresh categories so the
        # tag sensor (alfen_eve_tag_socket_1) works out of the box for existing installs.
        existing_cats = list(config_entry.options.get(CONF_REFRESH_CATEGORIES, DEFAULT_REFRESH_CATEGORIES))
        for cat in (CAT_LOGS, CAT_TRANSACTIONS):
            if cat not in existing_cats:
                existing_cats.append(cat)
        options = dict(config_entry.options)
        options[CONF_REFRESH_CATEGORIES] = existing_cats
        ...
        _LOGGER.debug("[%s] Migration to version 3 successful", log_id)
