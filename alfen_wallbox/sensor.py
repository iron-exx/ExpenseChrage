...
# Sekunden, die der "Tag Socket"-Sensor einen frisch vorgehaltenen RFID-Tag
# anzeigt, bevor er wieder auf "No Tag" (warten auf RFID) zurückfällt. Die
# Wallbox hält den letzten Tag sonst dauerhaft — dieser Reset macht daraus
# ein kurzes "erkannt", passend zum Tap-und-Laden-Ablauf.
_TAG_RESET_SECONDS: Final = 2.0
...
class AlfenSensor(AlfenEntity, SensorEntity):
    ...
    def __init__(
        ...
        # Zustand für den 2-Sekunden-Reset des Tag-Sensors
        self._tag_signature: str | None = None
        self._tag_seen_monotonic: float = 0.0
        self._tag_reset_unsub = None
    ...
    def _tag_with_reset(self, socket_number: int) -> str:
        """Tag-Wert des Sockets mit 2-Sekunden-Reset.

        Zeigt einen frisch vorgehaltenen Tag für _TAG_RESET_SECONDS an und
        fällt danach auf 'No Tag' zurück. Nötig, weil die Wallbox den letzten
        Tag sonst dauerhaft hält (Anzeige würde stundenlang kleben)."""
        latest = self.coordinator.device.latest_tag
        raw_tag = None
        raw_date = None
        if latest is not None:
            for key, value in latest.items():
                if key[0] == f"socket {socket_number}" and key[1] == "start":
                    if key[2] == "tag":
                        raw_tag = value
                    elif key[2] == "date":
                        raw_date = value

        if not raw_tag or raw_tag == "No Tag":
            return "No Tag"

        # Signatur aus Tag + Lese-Datum — erkennt auch erneutes Vorhalten
        # derselben Karte (Datum ändert sich, obwohl der Tag gleich bleibt).
        signature = f"{raw_tag}|{raw_date}"
        if signature != self._tag_signature:
            self._tag_signature = signature
            self._tag_seen_monotonic = time.monotonic()
            # HA nach dem Reset-Fenster erneut rendern lassen, damit ohne
            # Warten auf den nächsten Poll auf 'No Tag' umgeschaltet wird.
            if self.hass is not None:
                if self._tag_reset_unsub is not None:
                    self._tag_reset_unsub()
                self._tag_reset_unsub = async_call_later(
                    self.hass,
                    _TAG_RESET_SECONDS + 0.2,
                    self._tag_reset_callback,
                )

        if (time.monotonic() - self._tag_seen_monotonic) <= _TAG_RESET_SECONDS:
            return raw_tag
        return "No Tag"

    @callback
    def _tag_reset_callback(self, _now) -> None:
        """Nach dem Reset-Fenster: Sensor neu rendern → fällt auf 'No Tag'."""
        self._tag_reset_unsub = None
        self.async_write_ha_state()

    def _customTransactionCode(self, socker_number: int):
        if self.entity_description.key == f"custom_tag_socket_{socker_number}":
            return self._tag_with_reset(socker_number)
